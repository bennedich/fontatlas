...
SignedDistance QuadraticSegment::signedDistance(Point2 origin, double &param) const {
    ...
    {
		double distance = nonZeroSign( crossProduct( p[ 2 ] - p[ 1 ], p[ 2 ] - origin )) *
						  ( p[ 2 ] - origin ).length(); // distance from B
    ...
    for (int i = 0; i < solutions; ++i) {
		if ( t[ i ] > 0 && t[ i ] < 1 )
		{
			...
			double distance =
					nonZeroSign( crossProduct( p[ 2 ] - p[ 0 ], endpoint - origin )) * ( endpoint - origin ).length();
			if ( fabs( distance ) <= fabs( minDistance ))
			{
				minDistance = distance;
				param = t[ i ];
    ...
    if (param < .5)
		...
		return SignedDistance( minDistance,
							   fabs( dotProduct(( p[ 2 ] - p[ 1 ] ).normalize(), ( p[ 2 ] - origin ).normalize())));
}
...
SignedDistance CubicSegment::signedDistance(Point2 origin, double &param) const {
    ...
    param = -dotProduct(qa, ab)/dotProduct(ab, ab);
    {
		double distance = nonZeroSign( crossProduct( p[ 3 ] - p[ 2 ], p[ 3 ] - origin )) *
						  ( p[ 3 ] - origin ).length(); // distance from B
		if ( fabs( distance ) < fabs( minDistance ))
		{
			minDistance = distance;
    ...
    if (param < .5)
		...
		return SignedDistance( minDistance,
							   fabs( dotProduct(( p[ 3 ] - p[ 2 ] ).normalize(), ( p[ 3 ] - origin ).normalize())));
}
